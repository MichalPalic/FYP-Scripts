...
parser.add_argument('--resultdir',
                type=str,
                default="/home/michal/Desktop/spec_2017_rate_sweep_clear_192",
...
for ssit in [256, 512, 1024, 2048, 4096, 8192, 16384]:
    for lfst in [256, 512, 1024, 2048, 4096, 8192, 16384]:
        if float(ssit)/float(lfst) >= 0.25 and float(ssit)/float(lfst) <= 4:
            continue
        ...
        for checkpoint_path in checkpoint_paths:
            ...
            result_dir = f"{args.resultdir}/{ssit}_{lfst}/{spec_name}/{spec_idx}/{checkpoint_idx}"
            ...
            command.extend([f'{args.gem5dir}/build/X86/gem5' + ('.debug' if args.debug
                            ...
                            f"--lfst-size={lfst}",
                            f"--ssit-size={ssit}",
                            f"--store-set-clear-period={int( float(62464) * clearmult)}",
